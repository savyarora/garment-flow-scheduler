...
const ProcessesGrid: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return <Badge variant={config.variant} className={status === 'completed' ? 'bg-green-500 hover:bg-green-600' : status === 'in-progress' ? 'bg-blue-500 hover:bg-blue-600' : ''}>
        {config.label}
      </Badge>;
  };
  ...
  const toggleFreeze = (processId: string) => {
    setProcessData(prevData => prevData.map(process => process.id === processId ? {
      ...process,
  ...
  const saveScheduleChanges = () => {
    ...
    setProcessData(prevData => prevData.map(process => process.id === editingProcess ? {
      ...process,
  ...
  const resetToAutoSchedule = (processId: string) => {
    ...
    setProcessData(prevData => prevData.map(p => p.id === processId ? {
      ...p,
  ...
  const addSewingDay = () => {
    const lastDate = sewingSchedule.length > 0 ? sewingSchedule[sewingSchedule.length - 1].date : '2024-02-15';
  ...
  const updateSewingDay = (index: number, field: 'date' | 'quantity', value: string | number) => {
    setSewingSchedule(prev => prev.map((day, i) => i === index ? {
      ...day,
      [field]: value
    } : day));
  };
  ...
  const columnDefs: ColDef[] = [{
    ...
    field: 'processName',
    width: 200,
    cellRenderer: (params: any) => <div className="flex items-center gap-2 h-full">
          {getIcon(params.data.icon)}
          <span className={params.data.isPrimary ? 'font-bold text-blue-600' : 'font-medium'}>
            {params.value}
          </span>
          {params.data.isPrimary && <Badge variant="default" className="bg-blue-500 text-xs px-1 py-0">
              PRIMARY
            </Badge>}
          {params.data.isManualOverride && <Badge variant="outline" className="text-xs px-1 py-0 border-orange-300 text-orange-600">
              MANUAL
            </Badge>}
        </div>
  }, {
    ...
    field: 'description',
    flex: 1,
    cellRenderer: (params: any) => <div className="text-sm text-gray-600 py-2">
          {params.value}
        </div>
  }, {
    headerName: 'Schedule',
    ...
    cellRenderer: (params: any) => <div className="flex items-center gap-2 h-full">
          ...
          {!params.data.isPrimary && <Dialog>
              <DialogTrigger asChild>
                <Button size="sm" variant="ghost" onClick={() => openScheduleEditor(params.data)} className="h-6 w-6 p-0">
    ...
    field: 'offsetDays',
    width: 120,
    cellRenderer: (params: any) => <div className="flex items-center justify-center h-full">
          <Badge variant={params.value === 0 ? 'default' : 'outline'} className={params.value === 0 ? 'bg-blue-500' : params.value < 0 ? 'border-orange-300 text-orange-600' : 'border-green-300 text-green-600'}>
    ...
    field: 'duration',
    width: 130,
    cellRenderer: (params: any) => <div className="flex items-center gap-1 justify-center h-full">
    ...
    headerName: 'Controls',
    width: 120,
    cellRenderer: (params: any) => <div className="flex items-center gap-1 h-full">
          {!params.data.isPrimary && <>
              <Button size="sm" variant="ghost" onClick={() => toggleFreeze(params.data.id)} className="h-6 w-6 p-0" title={params.data.isFrozen ? 'Unfreeze schedule' : 'Freeze schedule'}>
                {params.data.isFrozen ? <Lock className="h-3 w-3 text-red-500" /> : <LockOpen className="h-3 w-3 text-gray-500" />}
              </Button>
              {params.data.isManualOverride && <Button size="sm" variant="ghost" onClick={() => resetToAutoSchedule(params.data.id)} className="h-6 w-6 p-0 text-blue-500" title="Reset to auto schedule">
                  ↻
                </Button>}
            </>}
        </div>
  ...
  return <div className="p-6 space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {sewingSchedule.map((day, index) => <div key={index} className="flex items-center gap-2">
                <Input type="date" value={day.date} onChange={e => updateSewingDay(index, 'date', e.target.value)} className="w-40" />
                <Input type="number" value={day.quantity} onChange={e => updateSewingDay(index, 'quantity', parseInt(e.target.value) || 0)} className="w-32" placeholder="Quantity" />
                <span className="text-sm text-gray-500">pcs</span>
                {sewingSchedule.length > 1 && <Button size="sm" variant="ghost" onClick={() => removeSewingDay(index)} className="text-red-500">
                    ×
                  </Button>}
              </div>)}
      ...
      <ScheduleGrid processData={processData} onProcessUpdate={handleProcessUpdate} onScheduleUpdate={handleScheduleUpdate} />

      {/* Main Grid */}
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-gray-800">Garment Manufacturing Process S</CardTitle>
        ...
        <CardContent>
          ...
          width: '100%'
            ...
            <AgGridReact rowData={processData} columnDefs={columnDefs} gridOptions={gridOptions} animateRows={true} />
          </div>
        </CardContent>
      </Card>

      {/* Schedule Editor Dialog */}
      {editingProcess && currentEditingProcess && <Dialog open={!!editingProcess} onOpenChange={() => setEditingProcess(null)}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Edit {currentEditingProcess.processName} Schedule</DialogTitle>
            </DialogHeader>
            <div className="space-y-3">
              {tempSchedule.map((day, index) => <div key={index} className="flex items-center gap-2">
              ...
              setTempSchedule(newSchedule);
                  ...
                  <Input type="number" value={day.quantity} onChange={e => {
              const newSchedule = [...tempSchedule];
              newSchedule[index] = {
                ...newSchedule[index],
            ...
            setTempSchedule(prev => [...prev, {
              ...
              <div className="flex gap-2 pt-4">
                ...
                <Button variant="outline" onClick={() => setEditingProcess(null)} className="flex-1">
                  Cancel
                </Button>
              </div>
            </div>
